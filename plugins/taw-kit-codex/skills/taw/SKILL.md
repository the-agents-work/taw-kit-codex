---
name: taw
description: Single Vietnamese-friendly entrypoint for taw-kit-codex. Use this WHENEVER a user describes a software project, feature request, bug, deploy, test, upgrade, cleanup, perf issue, rollback, refactor, type sync, seed, code review, stack swap, status check, security audit, or memory update — in Vietnamese OR English, with or without a /taw prefix. Triggers on prose like "lam cho toi shop ca phe" (build a coffee shop), "them trang lien he" (add contact page), "fix loi build", "deploy len vercel", "test login", "nang cap next 15", "don code", "lui lai ban hom qua", "kiem tra bao mat", "review code", "phan tich kien truc", and many more — see body for full trigger list. Classifies intent into BUILD / FIX / SHIP / MAINTAIN / ADVISOR and loads the matching branch file from `branches/`. SAFE mode (default) clarifies + waits for approval; YOLO mode (`yolo`/`lam luon`/`nhanh nha`/`auto` in prose) skips gates. ALWAYS reply in user's input language (Vietnamese for Vietnamese input).
---
