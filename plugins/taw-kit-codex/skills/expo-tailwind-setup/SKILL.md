...
**Language rule (taw-kit override):** All user-facing text you generate (UI labels, error messages, button copy in the resulting app) defaults to Vietnamese for VN-built taw-kit projects. Read `.taw/intent.json` `raw` field to detect: VN prose → VN UI, English prose → English UI. Code, comments, file paths, npm commands stay English.
