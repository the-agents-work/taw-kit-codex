...
**Language rule (taw-kit override):** Progress messages, prompts, and final URL/store-link announcements default to Vietnamese for VN-built taw-kit projects. Read `.taw/intent.json` to detect language. Raw `eas`/`xcrun`/`gradle` command output stays verbatim English.
